...
import bmartin127.advancedperipheralsremastered.lib.peripherals.IPeripheralCheck;
import bmartin127.advancedperipheralsremastered.lib.peripherals.IPeripheralFunction;
import bmartin127.advancedperipheralsremastered.lib.peripherals.IPeripheralOperation;
import bmartin127.advancedperipheralsremastered.lib.peripherals.IPeripheralPlugin;
...
public class OperationAbility implements IOwnerAbility, IPeripheralPlugin {
    ...
    protected void setCooldown(@NotNull IPeripheralOperation<?> operation, int cooldown) {
        if (cooldown > 0) {
            NbtCompound dataStorage = owner.getDataStorage();
            if (!dataStorage.contains(COOLDOWNS_TAG)) dataStorage.put(COOLDOWNS_TAG, new NbtCompound());
    ...
    protected int getCooldown(@NotNull IPeripheralOperation<?> operation) {
        ...
        NbtCompound cooldowns = dataStorage.getCompound(COOLDOWNS_TAG);
